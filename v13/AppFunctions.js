...
module.exports.deleteCommands = async (client) => {
    let cmdArrGlobal = await client.api
        ...
        .commands.get();
    cmdArrGlobal.forEach(async (i) => {
        const commandCheck = client.public.commands.get(i.name);
        if (!commandCheck) {
            await client.api.applications(client.user.id).commands(i.id).delete();
            console.log(`[Slash] Commande ${i.name} supprimé`);
        } else {
        }
    });
};
